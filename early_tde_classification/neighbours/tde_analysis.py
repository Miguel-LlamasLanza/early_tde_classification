...
def apply_further_cuts(features, amplitude_thresh = 9.5, chisq_cut = 500):

	features = features[(features.amplitude < amplitude_thresh) & features.r_chisq < chisq_cut]
	return features
...
def get_data(path):
	...
	data = apply_further_cuts(data, amplitude_thresh = 9.5, chisq_cut = 500)

	data.reset_index(inplace=True, drop=True)

	to_drop = {'objId', 'alertId', 'type', 'data_origin', 'ref_time', 'err_ref_time',
			'err_rise_time', 'err_amplitude', 'err_temperature', 'snr_amplitude', 'std_flux_r',
			'std_snr_g'}
